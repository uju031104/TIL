...
#### <!-- 26.07.24 -->
<details> 
  <summary>26.07.24</summary>
  <p>


최종 프로젝트 조 합류 완료 및 기획 시작   

간단하게 정리   

1. 인벤토리 & 크래프팅/제작 시스템   
마비노기풍 게임의 뼈대입니다. 단순히 아이템을 줍는 게 아니라 "데이터 구조와 상태 관리" 능력을 직관적으로 보여줄 수 있습니다.
- 발전시킬 기술 포인트   
  - Data-Driven Architecture (PrimaryDataAsset / DataTable): 아이템 조합법(Recipe), 재료 요구량, 도구 등급 등을 Hard-coding하지 않고 데이터 기반으로 확장성 있게 설계   
  - C++ UObject / Struct 기반 체계적인 아이템 데이터 관리: 아이템의 내구도, 품질(High/Normal), 상태(신선도 등) 변수를 효율적으로 메모리 내 관리   
  - 제작 메커니즘 (Crafting Logic): 단순히 "버튼 누르면 만들어짐"이 아니라, 제작 과정 미니게임이나 단계별 작업(재료 넣기 ➔ 망치질 ➔ 완성) 상태 머신 구축   

2. 채집, 필드 오브젝트 & 기믹/환경 상호작용
벌목, 광질, 낚시, 농사 등 세상과의 상호작용 파트입니다.   

발전시킬 기술 포인트   

- 컴포넌트 기반 상호작용 (UInteractionComponent): 나무, 바위, 낚시터 등 서로 다른 오브젝트가 하나의 C++ Interface/Component를 공유하도록 깔끔하게 구조화   

- 오브젝트 파괴 및 리스폰 시스템: 도구의 '등급(Tier)'에 따른 채집 가능 여부 판정, 파괴 애니메이션/피격 이펙트, 시간이 지나면 다시 생성되는 타이머/상태 관리   

- 자원 스폰/그리드 시스템: 농사나 하우징 시스템이 들어간다면, 맵 상의 특정 타일/그리드 위치에 오브젝트를 정렬하고 관리하는 spatial data structure 적용   

3. 생활/주민 AI 및 생활 루틴 (Behavior Tree)
- 마비노기 감성의 핵심은 "느긋하게 살아가는 세상"입니다. 몬스터뿐만 아니라 마을 NPC나 채집을 방해하는 생물들의 AI를 담당하는 것입니다.   

발전시킬 기술 포인트   

- 주민/동물 스케줄링 AI: 밤/낮 시간(Day/Night Cycle) 변화에 따라 이동하고, 작업하고, 쉬러 가는 BT(Behavior Tree) + Blackboard의 정밀한 상태 전환 logic.
- AIPerception을 활용한 자원 반응 AI: 플레이어가 나무를 베면 놀라서 도망치거나, 음식을 떨어뜨리면 먹으러 오는 주변 환경 반응형 AI.

상속보다는 컴포넌트(Composition) 구조 활용   

"채집 기능", "제작 기능", "상호작용 기능"을 Actor에 일일이 상속해서 만들지 않고, UActorComponent로 모듈화하여 필요한 곳에 붙이기만 하면 작동하도록 설계해 보세요   

C++ Interface (UInterface) 활용   

플레이어가 나무, 바위, NPC, 상자 등 완전히 다른 클래스를 바라보더라도 IInteractableInterface 하나로 통일해서 다루는 능력을 강조할 수 있습니다.   

Save / Load (유지성 데이터) 구축   

크래프팅 게임은 "내가 지은 집, 내 인벤토리, 필드 자원 상태"가 저장되는 게 핵심입니다. USaveGame 기반으로 필드 내 변화된 상태를 바이너리/JSON 등으로 효율적으로 세이브/로드하는 구조를 짜면 기술 점수가 대폭 올라갑니다.   
...
#### <!-- 26.08.07 -->
<details> 
  <summary>26.08.07</summary>
  <p>

GPT -> TRIPO -> AcuuRIG -> Blender(후처리) 작업을 거친 3D 캐릭터가 언리얼의 비정상적으로 그림자가 생기는 문제   

alt + 3으로 Unlit 상태로 보면 원래 만들었던 깔끔한 상태로 모이지만 빛이 들어오면 멍이 든 것 처럼 그림자가 들어오기 때문에 빛 처리 과정에서 뭔가 문제가 생겼을 가능성이 높음.   

`Shading Model`을 `Unlit`로 바꿔주고 `basecolor` 연결된 선을 `Emissive Color`로 바꿔주면 그림자가 사라진다.   
...
캐릭터를 멀리서 쳐다보면 엄청 자글자글해지는 현상   

TRIPO, AccuRIG, Blender에서 모두 이런 현상이 발생했었는데 최적화가 잘 못된 것인지 모르겠지만 여튼 이 부분에 대해서 찾아보았다.   

**원인은 바로 밉맵(Mipmap)**   

모든 프로그램에서 다 이렇게 보인다는 게 결정적 단서이다. GPU가 텍스처를 렌더할 때 자동으로 절반, 1/4, 1/8… 크기의 축소판을 미리 만들어두고, 오브젝트가 멀어지면 작은 걸 갖다 쓴다. 안 하면 화면이 지글지글 끓어서 훨씬 보기 싫어지기 때문에 어느 엔진에서든 기본으로 켜져 있다.   

최적화가 잘못 적용된게 아니라 정상 동작인데, TRIPO 모델은 유독 그 부작용이 심하게 드러나는 구조라고 한다.    

TRIPO 모델에서 특히 심한 이유   

자동 UV 언랩

TRIPO는 UV를 자동 생성하는데, 결과물이 수십~수백 개의 작은 UV 섬(island) 으로 산산조각 난다. 사람이 손으로 펴면 몸통 하나, 팔 하나로 크게 묶는데, 자동 알고리즘은 삼각형 단위로 쪼개버린다.   
실제로 블렌더에서 UV를 펼쳤을 때 정말 어디가 어딘지 모를 정도로 조각나있었다.   

문제는 밉맵을 만들 때 픽셀을 평균내면서 인접한 UV 섬끼리 색이 서로 번진다는 것. 섬 사이 여백(padding)이 보통 2~4픽셀뿐인데, 밉 레벨 3~4쯤 가면 그 여백이 0.5픽셀도 안 남아서 옆 섬의 색이 침범해서 지금과 같은 결과를 만들어낸다.    

**해결 방법1: 텍스쳐 Details 패널 설정**   
```
Mip Gen Settings   → Sharpen4  (기본 FromTextureGroup에서 변경)
Texture Group      → Character
LOD Bias           → -1        (한 단계 선명한 밉을 강제)
Never Stream       → 체크
```
...
r.AntiAliasingMethod 0    (none)
```

`r.AntiAliasingMethod 2`에서 기가막히게 자글자글한 현상이 줄어들었다.   
...
위에서 안티에일리어싱 세팅을 달리하면서 각각의 특징이 궁금해져서 좀 더 깊게 공부해보았다.   

**1. FXAA**   
완성된 화면에서 밝기 대비가 큰 경계선을 찾아 그 부분만 블러 처리. 순수 후처리라 3D 정보를 전혀 안 본다.   

**장점**   
- 압도적으로 저렴 (1080p에서 대략 0.1~0.3ms)
- 잔상(ghosting) 절대 없음, 움직여도 흐려지지 않음
- 프레임마다 결과가 동일 → 스크린샷·정지 이미지 뽑을 때 유리
- 하드웨어 요구사항 없음  
  

**단점**   
- 움직일 때 지글거림(shimmer/crawl)을 못 잡는다. 계단 현상만 뭉갤 뿐 원인은 그대로다.
- 얇은 지오메트리(철망, 머리카락, 전선) 처리 불가
- 텍스트와 UI가 뿌옇게 번진다.
- UE5에서 치명적인 문제: Lumen의 노이즈, Virtual Shadow Map, SSR, Nanite/폴리지의 디더링 LOD 전환은 전부 시간적 누적으로 정리되도록 설계돼 있다. FXAA는 누적을 안 하니 이 노이즈가 그대로 화면에 남는다. 지글지글한 점 패턴이 보인다.   
  
<br/>

**2. TAA**   

매 프레임 카메라를 픽셀 이하 단위로 미세하게 흔들면서 렌더 → 속도 벡터로 이전 프레임을 현재 위치에 맞춰 끌어와 누적. UE4 시절의 주력 기술이다.   

**장점**   
- 시간적 안정성이 뛰어남 — 지글거림·깜빡임을 잘 죽인다
- 서브픽셀 디테일 복원 가능 (여러 프레임 정보를 합치기 때문)
- 디더링·노이즈 정리 능력 우수
- TSR보다 저렴 (대략 0.5~1ms)
- 판정 로직이 단순해서 고대비·고주파 콘텐츠를 안 밀어낸다.


**단점**   
- 빠른 움직임에서 잔상이 꼬리처럼 남음
- 움직이는 동안 전체적으로 살짝 흐려짐
- 디오클루전 아티팩트: 움직이는 물체 뒤쪽에 참조할 히스토리가 없어 지저분해짐
- 업스케일링에 약함 — r.ScreenPercentage를 100 아래로 내리면 급격히 뭉개진다
- 속도 벡터가 없는 반투명 요소를 잘 못 다룸

<br/>

보정 노브   
```
r.TemporalAACurrentFrameWeight 0.15   기본 0.04, 올리면 선명 + 잔상↓, 지글↑
r.TemporalAASamples 8                 기본 8, 낮추면 반응 빠름 / 안정성↓
r.Tonemapper.Sharpen 0.7              뿌연 느낌 상쇄
```
...
하드웨어가 픽셀 하나에 대해 커버리지와 깊이를 여러 지점에서 샘플링한다. 셰이딩은 픽셀당 한 번만 하므로 슈퍼샘플링보다 싸다.   

**장점**   
- 가장 선명함. 시간적 블러도, 잔상도 없음
- 순수 지오메트리 경계 품질이 최상
- 결정론적 — 프레임마다 동일
- VR에 사실상 표준 (VR은 시간적 아티팩트에 특히 취약하고, 어차피 Forward를 씀)

**단점**   
- 기하학적 경계만 고친다. 스페큘러 앨리어싱, 셰이더 앨리어싱, 마스크 머티리얼(폴리지·헤어 카드) 경계는 그대로다.
- 대역폭·메모리 비용이 큼 (4x면 프레임버퍼가 4배)
- 시간적 누적이 없으니 Lumen 노이즈와 디더링이 남는다 (FXAA와 같은 문제)
- Forward Shading으로 넘어가면서 Deferred 전용 기능을 상당수 잃는다
- Nanite와의 궁합이 나쁘다

<br/>

**4. TSR**   
TAA의 발전형이면서 본질적인 업스케일러이다. 66% 해상도로 렌더해서 100%로 재구성하는 걸 전제로 설계됐다. 컴퓨트 셰이더 기반에 히스토리 채택/거부 판정이 훨씬 정교하다.

**장점**   
- 업스케일 상황에서 화질 대비 성능이 가장 우수 — 렌더 해상도를 낮춰 프레임을 크게 벌 수 있음
- Nanite·Lumen의 노이즈 정리 능력이 TAA보다 뛰어남
- 네이티브 해상도에서도 TAA보다 디테일 복원력이 좋음
- 플랫폼 독립적 (DLSS처럼 특정 GPU에 묶이지 않음)

**단점**   
- 네이티브 100%에서는 TAA보다 비쌈 (대략 1.5~3ms)
- 깜빡임 거부(shading rejection) 휴리스틱이 고대비·고주파 디테일을 오판해서 삭제합니다 ← 지금 겪은 문제가 정확히 이것
- 속도 벡터 정확도에 민감 — 반투명, World Position Offset 머티리얼에서 깨짐
- 에디터 뷰포트에서 프레임레이트가 낮으면 히스토리 품질이 같이 떨어짐
- r.ScreenPercentage 설정에 결과가 크게 좌우됨
  
<br/>

노브   
```
r.TSR.ShadingRejection.Flickering 0    깜빡임 판정 해제 — 디테일 삭제 방지
r.TSR.History.ScreenPercentage 200     히스토리 해상도 2배
r.TSR.Sharpen 0.5                      선명도 보강
```

<br/>

**결론**   
현재 프로젝트는 동적인 액션 게임이 아니고 캐릭터도 잘 다듬어져서 나오기 때문에 당장은 TAA를 유지하는 것이 좋아보인다.   
...
Import한 캐릭터로 리타겟팅 했을 때 캐릭터가 전반적으로 살짝 구부정한 자세로 있는 문제   

UE5의 자동 리타겟팅 기능의 신뢰도가 낮아서 생긴 문제였다.   
직접 크기를 조절해서 사이즈를 맞추고 다시 시도하니 등이 굽은 현상이 말끔하게 사라졌다.   

..인줄 알았으나 AccuRIG의 뼈대 형식은 UE5와 일치하지 않아서 이름이 겹치지 않아, 매핑이 제대로 되지 않는다.   

-> spine부터 neck까지 bone의 회전 각도를 조절해서 굽은 등을 펴줘서 고쳤다.   
...
**Motion Matching**과 관련된 3가지 Plugin을 깔아주고(Pose Search, Chooser, Motion Trajectory) `Pose Search Schema(PSS)`를 만들어준다.   

내부에 보면 위치, 속도 등을 이용해서 이동 예측을 하는 `Trajectory Channel`과 캐릭터 스켈레탈메쉬의 bone들의 위치정보(foot_l, foot_r)를 생산하는 `Pose Channel`이 있다.   

둘 다 가중치를(Weight) 둘 수 있고 기본값은 각각 7과 1이다.   

엔진에서 두 채널의 정보를 뽑아내서 현재 애니메이션과 예측되는 애니메이션의 유사성을 판단해서 다음에 나올 애니메이션을 결정한다고 보면 된다.   

이제 `Pose Search Database(PSD)`를 PSS 베이스로 만든 후 사용할 애니메이션들을 넣어준다.   
...
#### <!-- 26.08.13 -->
<details> 
  <summary>26.08.13</summary>
  <p>

언리얼 엔진의 스마트 포인터를 공부하는데 대부분은 기존 C++과 다를 것 없지만 `TSharedRef`라는 독특한 형태의 스마트 포인터를 알게 되었다.   

근데 스마트 '포인터'인데 이름은 Ref로 참조의 느낌을 주길래 왜 스마트 참조가 아닌지 궁금해서 찾아보았다.   

결론부터 말하면 TSharedRef는 C++의 참조(T&)가 전혀 아니고, 그냥 `null`이 금지된 포인터라고 한다.   

**C++ 참조와 다른 점**   

- C++ 참조는 한 번 바인딩되면 대상을 바꿀 수 없는데, `TSharedRef`는 바꿀 수 있다.   
- 주소를 가지지 않는 참조와 달리 `TSharedRef`는 크기가 16바이트인 값 타입이다.   
...
#### <!-- 26.09.04 -->
<details> 
  <summary>26.09.04</summary>
  <p>

머리에 아이템을 부착하는 시스템인데 기존에 단순하게 컴포넌트를 머리위에 놔두고 거기에 부착을 하는 방식을 택하였다.   

문제 상황   
캐릭터가 앉아도 부착된 아이템이 따라오지 않고 공중에 떠있는 현상이 발생했다.   

원인   
현재 캐릭터의 애니메이션은 뼈대가 움직이는 방식이고 앉았을때 skeletal mesh 자체가 내려가는게 아니라서 공중에 떠있는 것.   

해결 방법   
소켓을 사용해서 아이템을 해당 위치에 부착   

-> 하지만 회전값도 들어가서 자세가 바뀌면 머리위에 이쁘게 부착되는게 아니라 머리를 뚫는다..!   

문제 상황   
아이템 부착 이후 앉으면 머리를 뚫어버린다.   

원인   
회전값이 들어가서 자세에 따라 위치가 바뀌는 것으로 추정   

해결 방법    
부착 시 회전값을 제외한 Location만 Tick으로 체크를 해서 위치를 이쁘게 보정한다.   

-> 회전값이 사라졌지면 여전히 머리를 뚫는다.   

Tick을 켜서 보정도 해봤는데 정말 단순한 문제였다. 부모의 Mesh에 CarryComponent를 붙였던것.   
자식의 Mesh에 붙이니 바로 해결됐다...   
