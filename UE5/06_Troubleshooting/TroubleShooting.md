...
### [GAS] Anim Montage와 Gameplay Ability가 연동이 안되는 현상
...
**1. 발생한 현상**   
- 드롭킥을 날려서 래그돌이 된 캐릭터에게 다시 드롭킥을 날릴 수 있는 현상 + 두번 맞으면 일어나는 몽타주가 2번 재생되는 현상이 발생했다.   

**2. 원인**   
- 드롭킥을 맞고 날아갈때 아무런 예외처리를 하지 않아서 발생했다.    

**3. 해결방법**    
- GameplayTag를 활용하여 드롭킥에 맞고 날아가면 직접 무적 태그(State.Immunity)를 붙이고 일어서는 몽타주 재생 및 피를 회복하면 다시 떼주는 방식을 채택했다.   
...
**1. 발생한 현상**   
- `드롭킥 맞음 -> 래그돌 -> 누운 상태에서 일어서기`가 진행돼야하는데 `드롭킥 맞음 -> 래그돌 -> 일어남(0.1초) -> 누운 상태에서 일어서기`가 진행된다. 즉, 래그돌로 쓰러진 상태에서 0.1초동안 Idle(일어난 상태)로 확 바꼈다가 다시 누운 상태에서 일어서는 몽타주가 실행된다.   

**2. 원인**   
- 당장 보이는 원인을 찾을 수 없어서 여러가지 추측을 해보았다.   
  1. Montage를 실행하는 시점이 빨라서 Idle이 섞이는 경우   
  2. 래그돌 상태가 StandUp Montage와 잘 섞이지 않는 경우    

**3. 해결방법**    
- 위의 원인을 바탕으로 여러가지 해결법을 시도해보았다.   
  1. Montage Play를 가장 뒤로 미룬다.   
  2. 래그돌 상태를 캡처한 후 일어서는 애니메이션과 자연스럽게 이어주는 방식을 적용한다.      


<br/>

첫 번째: `Montage Play를 가장 뒤로 미룬다.`   
...
두 번째: `래그돌 상태를 캡처한 후 일어서는 애니메이션과 자연스럽게 이어주는 방식을 적용한다.`
...
안타깝게도 위의 방식도 전혀 소용없었다.   

세 번째: 정말 단순한 부분을 건드려봤다. 바로 애니메이션 몽타주의 Blend In 옵션이었다. 0.1~ 0.5까지 래그돌에서 일어서는 몽타주를 잘 섞이기 위해서 조절을 해봤다. 근데 여기서 매우 충격적이게도 이 옵션이 원인이었다.   

발생한 진짜 원인   
`Blend In`을 0.01이라도 설정하는 순간 `StandUp` 몽타주가 Idle 상태와 섞이려고 하는 것 이었다. 래그돌을 애니메이션이라고 착각하고 StandUp 몽타주를 래그돌과 잘 섞이게 하려고 Blend In 옵션을 기존 상태로 두거나 오히려 늘렸는데 이부분이 문제였다.
결국 Blend In을 0.0으로 설정해주는 순간 Idle과 섞이지 않기 때문에 며칠을 괴롭힌 위 현상이 말끔하게 사라졌다.   

<br/>

### [GAS] 래그돌 이후 StandUp 몽타주가 끝나도 무적이 계속 지속되거나 두번 쓰러진 후 HP가 0에서 회복되지 않는 현상

**1.발생한 현상**
- GA로 HitReact Montage를 실행하게 설계를 했는데 몽타주가 끝나고도 무적이 계속 지속되는 현상 + 두번째 쓰러짐 이후 피 회복이 안되는 현상이 발생했다.   

**2.원인**    
- 수동으로 태그를 붙였다 뗐다 하다보니 내부 로직이 꼬인 것으로 추정된다.    
...
### [GAS] Item 사용 시 AttributeSet 값이 제대로 반영되지않는 현상

**1.발생한 현상**
- 스피드업, 스피드다운 아이템 사용 시 실제 속도에 반영되지 않는 현상이 발생했다.   

**2.원인**    
- 기존에는 단순 델리게이트를 사용해서 바꿔주었다.   
...
기존 방식의 문제점은 `MaxWalkSpeed`가 기존 값으로 계속 덮어씌워지는 것이었다. 왜냐하면 `CharacterMovementComponent`는 내부에 서버-클라이언트 간의 이동을 동기화하는 복잡한 네트워크 예측(Prediction) 로직이 들어있다. 그래서 단순히 통보하는 Delegate 방식을 사용했을 때 타이밍이 꼬여 기존 값을 덮어씌우는 현상이 나타나는것이다.   
...
### [GAS] 나이아가라 이펙트가 DropKick 타격 지점에서 재생되지 않는 현상

**1.발생한 현상**
- 나이아가라 이펙트가 DropKick 타격 지점에서 재생되지 않고 예외처리 해둔 골반(Hips)에서 실행되는 현상이 발생했다.   

**2.원인**    
- HitResult가 비어있거나 HitResult에 타겟 정보가 제대로 들어있지 않거나 전달이 안된 상황이다.       

**3.해결방법**   
- HitResult를 Context에 바인딩해서 전달을 해준다.       
...
하지만 여전히 해결되지 않았다. 높은 확률로 HitResult 자체가 비어있거나 존재하지 않는 듯 하다.   
따라서 디버깅을 통해서 HitResult의 어떤 부분이 없는지 제대로 확인해봐야한다.   
...
디버깅 해봤더니 [DropKick] Payload.ContextHandle 자체가 유효하지 않음이 떴다. 그래서 정보가 전달되는 ANS(Trace로 타격 지점 결과를 줌)로 가서 Context에 직접 HitResult를 채워줬다.   
...
수정한 결과 정확히 발로 타격한 지점에서 나이아가라 이펙트가 재생됐다.   

<br/>

## [Physics] 네트워크 환경에서 축구공의 물리 구현 R&D
...
### [Physics] 네트워크 환경에서 축구공의 물리 구현 R&D(결과 요약)   
...
### [Physics] 네트워크 환경에서 축구공의 물리 구현 R&D(자세한 로직)
---   
...
### [Ragdoll] 드롭킥 피격 캐릭터가 제자리에 주저앉는 현상
...
### [Ragdoll] 래그돌 종료 후 기상 애님몽타주가 실행될 때 오류
...
### [Ragdoll] 래그돌 상태에서 실행한 EnableRagdoll() 상태에서 못벗어나고 가만히 있는 버그

<br/>

**1. 발생한 현상**   
- 팀원들이랑 3대3으로 접속해서 테스트하는데 드롭킥을 맞고 래그돌이 된 후 일어섰을때 래그돌 상태에서 실행한 EnableRagdoll() 상태에서 못벗어나고 가만히 있는 버그가 발생했다.   

**2. 원인**   
- 기존 코드는 5초이후 래그돌이 풀리게 설정됐다. 하지만 아이템으로 파워가 커지거나 특정 상황에서 래그돌을 맞고 높게 날아가버려서 5초가 지나면 로직이 꼬일 가능성이 매우 높다.   
    
**3. 해결 방법**    
- 실제로 캐릭터가 래그돌 상태에서 바닥에 닿고 속도가 0에 가까운 경우 래그돌이 풀리게 로직을 수정했다.   
...
### [Ragdoll] 네트워크 환경에서 캐릭터 래그돌 R&D(결과 요약)   
